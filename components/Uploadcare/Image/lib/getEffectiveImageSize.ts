...
const getEffectiveWidth = (width: number, height: number, imageDetails?: Record<string, any>) => {
...
const getEffectiveHeight = (width: number, height: number, imageDetails?: Record<string, any>) => {
    const aspectRatio = imageDetails
        ? imageDetails.original_width / imageDetails.original_height
        : null;
